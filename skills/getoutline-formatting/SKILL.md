...
description: Use this skill when creating, editing, or formatting documents in a getOutline wiki (https://www.getoutline.com), typically reached via an MCP tool such as mcp__<your-wiki>__create_document / update_document / fetch. Activate whenever the user asks to write into the wiki, update a wiki document, "schreibe das ins wiki", "logbuch eintrag", references a getOutline document ID or URL slug, or wants a wiki page made human-friendly and easier to read. Covers Outline-specific markdown extensions that plain markdown does not have — coloured info/warning/tip/success callouts, collapsible toggle sections written with `++++` fences, mermaid diagrams that must use the ```mermaidjs language tag (NOT plain ```mermaid as in vanilla markdown), dark-mode mermaid theming, multi-colour text highlighting via `==text==`, underline via `__text__`, inline and block LaTeX math, `diff` code blocks for showing changes, file attachments, embeds (Figma/YouTube/GitHub/Linear/Google Suite/etc.), right-aligned numeric tables, and the writing style that makes a wiki page genuinely scannable for a human reader (short sentences, clear sections, callouts for the punchline, tables for evidence, toggles for optional detail). Also covers the editMode strategy for the update_document MCP call (patch vs replace vs append) so existing rich formatting like highlights and comments is preserved.
---
